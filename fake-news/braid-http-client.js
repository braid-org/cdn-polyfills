    ...
    return await new Promise((done, fail) => {
        ...
        async function connect() {
            ...
            try {
                ...
                // - a `fetcher` was supplied           → off: the fetcher
                //   is the transport, e.g. another Durable Object's stub
                ...
                try {
                    if (!params.fetcher
                        && !disabled_multiplex_hosts.has(get_origin(url))
                        && (mux_params === true
                            || (mux_params !== false
                        ...
                        // Or do a regular fetch, over the caller's transport
                        // if it gave one.
                        res = await (params.fetcher ?? normal_fetch)(url, params)
                } catch (e) {
                ...
                function start_subscription (cb, error, options = {}) {
                    subscription_cb = cb
                    ...
                    handle_fetch_multiresponse(
                        ...
                        async (update) => {
                            ...
                            // A 1xx sub-response, such as 104 Origin Status,
                            // is status rather than an update: it goes to
                            // on_status, in stream order, never to the
                            // update callback
                            var informational = update.status >= 100
                                                && update.status < 200,
                                deliver = informational
                                          ? (options.on_status ?? options.onStatus
                                             ?? (() => {}))
                                          : cb
                            try {
                ...
                if (subscription_cb && res.ok)
                    start_subscription(subscription_cb, subscription_error,
...
// The multiplexing protocol, apart from any HTTP library: shared by this
// client, by braidify, and by intermediaries such as a CDN polyfill that
// terminate or relay multiplexers.  Spec: https://braid.org/protocol/multiplexing
//
// A multiplexer is created with POST /.well-known/multiplexer/<id>, whose
// response is a stream that stays open.  A request carrying
// Multiplex-Through: /.well-known/multiplexer/<id>/<rid> is answered 293,
// and its real response is written into that stream as frames:
//
//     start response <rid>\r\n
//     <n> bytes for response <rid>\r\n<n bytes>
//     close response <rid>\r\n
//
// with bare CRLFs between frames as heartbeats.  A response's first bytes
// are its status line and header section, then come its body's.
// DELETE /.well-known/multiplexer/<id>/<rid> closes one response.
var multiplexing = (() => {
    var version = '1.0',
        well_known = '/.well-known/multiplexer/',
        id_pattern = '[A-Za-z0-9_-]+',
        encoder = new TextEncoder(),
        decoder = new TextDecoder()

    // Which multiplexing request this is, or null: {kind: 'create' | 'close'
    // | 'through', id, rid, version}.  `header(name)` gives a request
    // header's value.  The version comes back unchecked: the caller answers
    // a mismatch on the well-known routes with 400, and treats a
    // Multiplex-Through request with the wrong version as not multiplexed.
    function route (method, path, header) {
        var version = header('multiplex-version'),
            at_well_known = new RegExp(`^${well_known}(${id_pattern})(?:/(${id_pattern}))?$`)
                            .exec(path),
            legacy = method === 'MULTIPLEX'
                     && new RegExp(`^/(${id_pattern})(?:/(${id_pattern}))?$`)
                        .exec(path)
        // The MULTIPLEX method, an earlier form of the well-known routes:
        // MULTIPLEX /<id> creates, MULTIPLEX /<id>/<rid> closes
        if (legacy)
            return legacy[2] ? {kind: 'close', id: legacy[1], rid: legacy[2], version}
                             : {kind: 'create', id: legacy[1], version}
        if (at_well_known && method === 'POST' && !at_well_known[2])
            return {kind: 'create', id: at_well_known[1], version}
        if (at_well_known && method === 'DELETE' && at_well_known[2])
            return {kind: 'close', id: at_well_known[1], rid: at_well_known[2],
                    version}
        var through = new RegExp(`^${well_known}(${id_pattern})/(${id_pattern})$`)
                      .exec(header('multiplex-through') ?? '')
        if (through)
            return {kind: 'through', id: through[1], rid: through[2], version}
        return null
    }

    // ---- The frames of a multiplexer's stream ----

    function line (kind, rid) {
        return encoder.encode(`${kind} response ${rid}\r\n`)
    }
    function frame_line (rid, byte_length) {
        return encoder.encode(`${byte_length} bytes for response ${rid}\r\n`)
    }
    function frame (rid, bytes) {
        bytes = to_bytes(bytes)
        return concat([frame_line(rid, bytes.byteLength), bytes])
    }

    // Splits a multiplexer's stream into events as its chunks are fed,
    // however the chunk boundaries fall: {kind: 'start' | 'close', rid} and
    // {kind: 'bytes', rid, bytes}.  Throws on a malformed frame.
    var frame_pattern = new RegExp(
            `^((\\d+) bytes for|close|start) response (${id_pattern})\\r?\\n$`),
        longest_line = 256
    function parser () {
        var input = new Uint8Array(0),
            pending = null   // {rid, size} while a frame's bytes are awaited
        return {
            feed (chunk) {
                var events = []
                chunk = to_bytes(chunk)
                input = input.length ? concat([input, chunk]) : chunk
                for (;;) {
                    if (pending) {
                        if (input.length < pending.size) break
                        events.push({kind: 'bytes', rid: pending.rid,
                                     bytes: input.slice(0, pending.size)})
                        input = input.subarray(pending.size)
                        pending = null
                        continue
                    }
                    // Bare CRLFs between frames are heartbeats
                    var start = 0
                    while (start < input.length
                           && (input[start] === 13 || input[start] === 10))
                        start++
                    var end = input.indexOf(10, start)
                    if (end === -1) {
                        if (input.length - start > longest_line)
                            throw new Error('invalid multiplex frame: no line'
                                            + ` within ${longest_line} bytes`)
                        input = input.subarray(start)
                        break
                    }
                    var text = decoder.decode(input.subarray(start, end + 1)),
                        match = frame_pattern.exec(text)
                    if (!match)
                        throw new Error('invalid multiplex frame: '
                                        + JSON.stringify(text))
                    input = input.subarray(end + 1)
                    if (match[1] === 'start' || match[1] === 'close')
                        events.push({kind: match[1], rid: match[3]})
                    else pending = {rid: match[3], size: parseInt(match[2])}
                }
                return events
            }
        }
    ...
    // ---- A multiplexed response's status line and header section ----

    function serialize_status_and_headers ({status, statusText, headers}) {
    ...
    // {status, headers (names lowercased), rest (the bytes after)}, or null
    // until the blank line ending the section has arrived.  The status line
    // may carry the protocol version or not.
    function parse_status_and_headers (bytes) {
        bytes = to_bytes(bytes)
        var blank = blank_line(bytes)
        if (!blank) return null
        var lines = decoder.decode(bytes.subarray(0, blank.at)).split(/\r?\n/),
            status = parseInt(/^(?:HTTP\/\S+ )?(\d{3})/.exec(lines[0])?.[1]),
            headers = {}
        for (var header of lines.slice(1)) {
            var colon = header.indexOf(':')
            if (colon > 0)
                headers[header.slice(0, colon).trim().toLowerCase()] =
                    header.slice(colon + 1).trim()
        }
        return {status, headers, rest: bytes.subarray(blank.after)}
    }

    // The first line ending followed by another: {at, after}, or null
    function blank_line (bytes) {
        for (var i = 0; i < bytes.length; i++) {
            if (bytes[i] !== 10) continue
            if (bytes[i + 1] === 10) return {at: i, after: i + 2}
            if (bytes[i + 1] === 13 && bytes[i + 2] === 10)
                return {at: i, after: i + 3}
        }
        return null
    }

    // ---- The responses of the well-known routes and of Multiplex-Through ----
    //
    // Each is {status, statusText, headers, body}, for a server to write as
    // it likes.  The multiplexer routes are meant to be reachable from any
    // page, so their responses carry CORS-free headers.  The 293 belongs to
    // the application's URL and carries none: the application's own CORS
    // headers go on it.
    var cors_free_headers = {
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': '*',
        'Access-Control-Allow-Headers': '*',
        'Access-Control-Expose-Headers': '*'
    }
    var versioned = headers => ({'Multiplex-Version': version, ...headers})
    var responses = {
        // The multiplexer's own stream.  Its media type and no-transform
        // keep a compressing intermediary from holding small frames back
        // in its compressor until it has a block's worth.
        created: () => ({status: 200, statusText: 'OK', headers: versioned({
            'Incremental': '?1',
            'Content-Type': 'application/octet-stream',
            'Cache-Control': 'no-store, no-transform',
            ...cors_free_headers
        })}),
        closed: () => ({status: 200, statusText: 'OK',
                        headers: versioned(cors_free_headers), body: ''}),
        via_multiplexer: through => ({
            status: 293, statusText: 'Responded via multiplexer',
            headers: versioned({'Multiplex-Through': through,
                                'Cache-Control': 'no-store'}),
            body: 'Ok.'
        }),
        bad_version: () => ({status: 400, statusText: 'Bad Multiplexer Version',
                             headers: versioned(cors_free_headers), body: ''}),
        duplicate_multiplexer: id => conflict('Multiplexer already exists',
            `Cannot create duplicate multiplexer with ID '${id}'`),
        duplicate_request: (id, rid) => conflict('Request already multiplexed',
            `Cannot multiplex request with duplicate ID '${rid}' `
            + `for multiplexer '${id}'`),
        // A DELETE naming what is not there
        unknown_multiplexer_on_close: id => ({
            status: 404, statusText: 'Multiplexer not found',
            headers: versioned({'Bad-Multiplexer': id, ...cors_free_headers}),
            body: `multiplexer ${id} does not exist`
        }),
        unknown_request_on_close: rid => ({
            status: 404, statusText: 'Multiplexed request not found',
            headers: versioned({'Bad-Request': rid, ...cors_free_headers}),
            body: `request ${rid} does not exist`
        }),
        // A Multiplex-Through naming a multiplexer that is not there, which
        // may be on its way: the client retries
        unknown_multiplexer_on_through: id => ({
            status: 424, statusText: 'Multiplexer not found',
            headers: versioned({'Bad-Multiplexer': id, ...cors_free_headers}),
            body: `multiplexer ${id} does not exist`
        })
    }
    function conflict (error, details) {
        return {status: 409, statusText: 'Conflict',
                headers: versioned({'Content-Type': 'application/json',
                                    ...cors_free_headers}),
    ...
    function concat (parts) {
        var out = new Uint8Array(parts.reduce((n, part) => n + part.byteLength, 0)),
            offset = 0
    ...
    return {
        version, well_known, route,
        is_well_known: path => path.startsWith(well_known),
        line, frame_line, frame, parser,
        serialize_status_and_headers, parse_status_and_headers,
        responses, cors_free_headers
    }
})()
...
async function create_multiplexer(origin, mux_key, params, mux_params, attempt) {
    var multiplex_version = multiplexing.version
...
// Reads a multiplexer's stream and calls cb(rid, bytes) for each frame:
// an empty Uint8Array when a response starts, its bytes as they come, and
// undefined when it closes
async function parse_multiplex_stream(reader, cb, on_error) {
    try {
        var frames = multiplexing.parser()
        while (true) {
            var { done, value } = await reader.read()
            if (done) throw new Error('multiplex stream ended unexpectedly')
            for (var event of frames.feed(value))
                cb(event.rid, event.kind === 'start' ? new Uint8Array()
                              : event.kind === 'close' ? undefined
                              : event.bytes)
        }
    } catch (e) { on_error(e) }
}
...
// What one HTTP status means for the request that got it: 'online' (a
// subscription is established), 'poll' or 'poll-noop' (a plain answer to a
// GET), 'acked' (a write landed), 'retry' (transient, try this request
// again after a delay) or 'give_up'.  http_bus acts on it, and an
// intermediary holding subscriptions of its own can classify the same way.
...
// Reachable from the globals a browser script tag defines, too
braid_fetch.multiplexing = multiplexing
http_bus.classify_response_status = classify_response_status
